...
import { Select, SelectTrigger, SelectValue, SelectContent, SelectItem, SelectGroup } from "@/app/components/ui/select"
...
const GeneratePoster = ({ userId }: { userId: string }) => {
  ...
  const [editMode, setEditMode] = useState<"none" | "headline" | "cta" | "color">("none")
  ...
  const applyEdit = async () => {
    ...
    const res = await fetch("/api/poster-edit", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-5">
          ...
          <div>
            ...
            <Input {...register("subHeadline")} placeholder="Tuliskan Sub Headline" />
          </div>
          ...
          <Button onClick={handleSubmit(onSubmit)} disabled={loading} className="w-full py-5">
      ...
      <Card className="flex items-center justify-center min-h-[500px]">
        <CardContent>
          ...
          {!generatedImage && (
            <p className="text-gray-500">Poster akan muncul di sini setelah digenerate.</p>
          )}

          {generatedImage && (
            <div className="space-y-4">

              <Image src={generatedImage} alt="Poster" width={600} height={600} className="rounded shadow mx-auto" />

              {/* EDIT BUTTONS */}
              <div className="flex gap-3 justify-center">
                <Button onClick={() => { setEditMode("headline"); setEditValue(headline) }} variant="outline">Edit Headline</Button>
                <Button onClick={() => { setEditMode("cta"); setEditValue(cta) }} variant="outline">Edit CTA</Button>
                <Button onClick={() => { setEditMode("color"); setEditValue(theme) }} variant="outline">Edit Tema</Button>
              </div>
              ...
              {editMode !== "none" && (
                <div className="border p-4 rounded space-y-3">
                  ...
                  {editMode === "color" ? (
                    <select className="w-full border px-3 py-2 rounded" value={editValue} onChange={(e) => setEditValue(e.target.value)}>
                      {themes.map((t) => (
                  ...
                  <Button onClick={applyEdit} disabled={loading} className="w-full">
