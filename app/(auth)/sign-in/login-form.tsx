...
import { loginWithEmail, loginWithGoogle } from '@/app/service/authService'
import { toast } from 'sonner'
import Spinner from '@/app/components/ui/spinner'

type validateLogin = z.infer<typeof authSchemaWithEmail>

const LoginForm = () => {
  const { register, handleSubmit, formState: {errors}} = useForm<validateLogin>({
  ...
  const loginHandle = async (formData: validateLogin) => {
    try {
      setIsSubmitting(true)
      const { data, error } = await loginWithEmail(formData)
  ...
  return (
    <div className="grid min-h-svh lg:grid-cols-2">
            ...
            <form onSubmit={handleSubmit(loginHandle)} className="flex flex-col">
              <div className="flex justify-center mb-3">
                <Link href="/" className="flex items-center font-medium">
                  <Image src={'/images/app-logo.png'} width={0} height={0} sizes="100vw" className="h-11 w-auto" alt={'logo rebox'} />
                </Link>
              </div>
              ...
              <p className="text-center mt-3 text-muted-foreground text-sm text-balance">
                Tidak punya akun?, <Link className="text-primary underline" href={'/sign-up'}>Registrasi Sekarang</Link>
              </p>
            </form>
          </div>
        </div>
      ...
      <div className="bg-muted z-10 relative hidden lg:block">
        <Image src={'/images/auth-img.png'} fill className="object-cover" alt={'authentication image'} />
      </div>
    </div>
  )
}
